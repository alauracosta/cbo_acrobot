...
def init_hebo_filling(n_seed = 10, n_iter = 20):
    ...
    opt = HEBO(space, rand_sample=n_seed*5, acq_cls = EI_mine)
    ...
    for i in range(n_seed*5):
        ...
        vals = rec.iloc[0]
        res = control_acrobot(vals['kp'], vals['ki'], vals['kd'], target_height=vals['height'])
        print(f"Random Sampling iter {i+1}, : params={vals.values.tolist()}, reward {res:.2f}")
        opt.observe(rec, np.array([[-res]]))
        x_init.append(vals.to_dict())
        y_init.append(-res)
    ...
    optFinal = HEBO_VectorContextual(space, context_dict={
        'ctx_1.0': {'height': 1.0},
        'ctx_1.2': {'height': 1.2},
        'ctx_0.8': {'height': 0.8},
        'ctx_0.6': {'height': 0.6},
    ...
    optFinal.hebo.acq_cls = EI_mine

    x_init = pd.DataFrame(x_init)
    y_init = np.array(y_init).reshape(-1,1)
    optFinal.observe_new_data(x_init, y_init)
